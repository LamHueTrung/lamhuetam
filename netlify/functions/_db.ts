...
const TransactionSchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "tx-" + Math.random().toString(36).substr(2, 9),
    },
...
const DebtSchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "debt-" + Math.random().toString(36).substr(2, 9),
    },
    type: {
      type: String,
      enum: ["installment", "credit_card", "friend"],
      required: true,
    },
...
const SavingsGoalSchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "save-" + Math.random().toString(36).substr(2, 9),
    },
...
const FixedExpenseCategorySchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "fec-" + Math.random().toString(36).substr(2, 9),
    },
    name: { type: String, required: true },
...
const FixedExpenseTaskSchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "fet-" + Math.random().toString(36).substr(2, 9),
    },
...
const DiaryEntrySchema = new mongoose.Schema(
  {
    id: {
      type: String,
      default: () => "diary-" + Math.random().toString(36).substr(2, 9),
    },
    date: { type: String, required: true },
    content: { type: String, required: true },
    mood: {
...
const UserProfileSchema = new mongoose.Schema(
  {
    ...
    livingContext: {
      type: String,
      default:
        "Ở quê (Tiểu Cần - Trà Vinh), di chuyển xa để đi làm, áp lực gia đình và tiền bạc thường trực.",
    },
    currentJob: {
      type: String,
      default: "Lập trình viên tại Rynan Technologies",
    },
    ...
    skills: {
      ...
      foundation: {
        type: String,
        default: "Từng có nền tảng và tư duy tốt về UI/UX kết hợp API",
      },
      usedTech: {
        type: [String],
        default: [
      ...
      currentWorry: {
        type: String,
        default:
          'Cảm giác bị "mất gốc" Node.js vì môi trường hiện tại không còn phù hợp định hướng dài hạn',
      },
    },
    education: {
      ...
      status: {
        type: String,
        default: "Chưa làm lễ tốt nghiệp, đã lấy bằng tốt nghiệp",
      },
    },
...
export const SalaryConfig =
  ...
  mongoose.model("SalaryConfig", SalaryConfigSchema);
export const FixedExpenseCategory =
  mongoose.models.FixedExpenseCategory ||
  mongoose.model("FixedExpenseCategory", FixedExpenseCategorySchema);
export const FixedExpenseTask =
  mongoose.models.FixedExpenseTask ||
  mongoose.model("FixedExpenseTask", FixedExpenseTaskSchema);
...
const AIConfigSchema = new mongoose.Schema(
  {
    ...
    baseUrl: {
      type: String,
      default: "https://trungsaas-beta.onrender.com/v1",
    },
