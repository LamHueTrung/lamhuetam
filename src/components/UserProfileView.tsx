...
export default function UserProfileView({
  ...
  // Collapse sections states (string keys)
  ...
  const handleTestAiConnection = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setTestMessage(data.message || "Kết nối thành công!");
      ...
      setTestMessage(err.message || "Lỗi mạng hoặc không gọi được server");
      toast.error("Lỗi mạng khi thử kết nối AI");
    } finally {
  ...
  const handleSaveAiConfig = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toast.error("Lỗi khi lưu: " + (data.error || "Lỗi server"));
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="space-y-3.5">
        ...
        {/* 1.1: Thông tin cá nhân & Liên hệ (Hợp nhất) */}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  Lý lịch, số điện thoại, email và bối cảnh sống
                </p>
              </div>
            </div>
            {!isEditing && (
              ...
              >
                {isEditing ? (
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    {/* Email list preview */}
                    <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-1.5">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                        Danh sách Email liên hệ ({emails.length})
                      </span>
                      <div className="flex flex-wrap gap-2 pt-0.5">
                        {emails.length > 0 ? (
                          emails.map((email, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold px-2.5 py-1 rounded-xl border border-slate-200 dark:border-slate-700"
                            >
                              <Icon
                                path={mdiEmailOutline}
                                size={0.6}
                              ...
                              <span>{email}</span>
                              {idx === 0 && (
                                <span className="text-[9px] bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-300 font-bold px-1.5 py-0.2 rounded">
                                  Chính
                                </span>
                              )}
                            </span>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  Thế mạnh kỹ thuật, tech stack & định hướng công việc
                </p>
              </div>
            </div>
            {!isEditing && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  Trường đào tạo và trạng thái bằng cấp
                </p>
              </div>
            </div>
            {!isEditing && (
        ...
        {/* 1.4: Định hướng & Thông tin bổ sung */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
          <div
      ...
      {/* ========================================================================= */}
      {/* 💳 NHÓM 2: CÀI ĐẶT TÀI CHÍNH & THẺ (Financial Settings) */}
      {/* ========================================================================= */}
      <div className="space-y-3.5 pt-2">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-blue-100 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400">
              <Icon path={mdiCreditCardOutline} size={0.75} />
            </div>
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Cài Đặt Tài Chính & Thẻ
              </h2>
            </div>
          </div>
          <span className="text-[10px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full">
        ...
        {/* 2.1: Cấu hình Thẻ tín dụng & Ngày sao kê */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
          <div
      ...
      <div className="space-y-3.5 pt-2">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  Chọn mô hình AI, nhà cung cấp và API Key cá nhân
                </p>
              </div>
            </div>
            <Icon
        ...
        <CacheManagerSection
          ...
          onToggle={() => toggleSection("system_cache")}
          onResyncRequired={onResyncData}
        />

        {/* 3.3: Phiên bản & Cập nhật PWA */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3">
          <div
          ...
          >
            <div className="flex items-center gap-2">
              <div className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                <Icon path={mdiPackageUp} size={0.9} />
              </div>
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <p className="text-[11px] text-slate-400">
                  Kiểm tra cập nhật PWA và phiên bản triển khai mới nhất
                </p>
              </div>
            </div>
            <Icon
              ...
              >
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800">
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                    className="w-full sm:w-auto bg-gradient-to-r from-amber-500 to-rose-500 text-white text-xs font-bold px-4 py-2 rounded-xl shadow-sm cursor-pointer flex items-center justify-center gap-1.5 hover:opacity-90 transition-all disabled:opacity-60"
                  >
                    <Icon
      ...
      {/* Action Footer (Khi đang ở chế độ Edit) */}
