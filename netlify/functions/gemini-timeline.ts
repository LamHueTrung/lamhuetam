...
export const handler: Handler = async (event) => {
  ...
  try {
    const { partner, amtPerPeriod, installments, startDate, paymentDueDay } =
    ...
    if (!apiKey) {
      ...
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ timeline: fallback, fromAI: false }),
      };
    }
    ...
    const response = await fetch(`${BASE_URL()}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Chỉ trả về mảng JSON thuần, không text thêm.",
          },
    ...
    if (!response.ok) {
      ...
      return {
        ...
        body: JSON.stringify({
          timeline: fallback,
          fromAI: false,
          note: "AI lỗi, dùng lộ trình mặc định",
        }),
      };
    }
    ...
    const timeline = Array.isArray(parsed)
      ? parsed
      : parsed.timeline || parsed.installments || [];
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ timeline, fromAI: true }),
    };
  } catch (error: any) {
    console.error("gemini-timeline error:", error);
    const { startDate, installments, amtPerPeriod, paymentDueDay } = JSON.parse(
      event.body || "{}",
    );
    const fallback = generateDefaultTimeline(
    ...
    return {
      ...
      body: JSON.stringify({
        timeline: fallback,
        fromAI: false,
        note: "Đã dùng lộ trình mặc định",
      }),
    };
  }
};
