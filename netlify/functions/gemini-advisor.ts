  ...
  try {
    ...
    if (!apiKey) {
      return {
        ...
        body: JSON.stringify({
          ...
          message:
            "Thiếu API Key. Hãy cấu hình API Key trong trang Thông tin cá nhân hoặc biến môi trường Netlify.",
        }),
      };
    }
    ...
    const totalDebt =
      debts
        ...
        ?.reduce((sum: number, d: any) => sum + (d.currentBalance || 0), 0) ||
      0;
    const totalMonthlyPayment =
      debts
        ?.filter((d: any) => d.status === "active")
        ?.reduce((sum: number, d: any) => sum + (d.monthlyPayment || 0), 0) ||
      0;
...
`
    ...
    const lastMonthIncome =
      transactions
        ?.filter(
          (t: any) => t.type === "income" && t.date?.startsWith(lastMonth),
        )
        ?.reduce((s: number, t: any) => s + Number(t.amount), 0) || 0;
    const lastMonthExpense =
      transactions
        ?.filter(
          (t: any) => t.type === "expense" && t.date?.startsWith(lastMonth),
        )
        ?.reduce((s: number, t: any) => s + Number(t.amount), 0) || 0;
        ...
        messages: [
          {
            role: "system",
            content:
              "Bạn là Gemini Co-Visor, cố vấn tài chính và sự nghiệp cá nhân cao cấp dành riêng cho Lâm Huệ Trung. Luôn trả lời bằng tiếng Việt, dùng Markdown, gạch đầu dòng, số liệu cụ thể. Tư vấn thực tế, sâu sắc, thấu hiểu bối cảnh sống và định hướng sự nghiệp.",
          },
    ...
    if (
      ...
      message =
        "API_KEY không hợp lệ. Kiểm tra lại key hoặc dùng OpenRouter key (sk-or-...).";
    } else if (
      ...
      message =
        "Đã hết quota. Vui lòng kiểm tra lại cấu hình hoặc quota tài khoản AI của bạn.";
    } else {
      message = errMsg;
    }
    return {
