...
export default function Navbar({
  ...
  return (
    <div
      id="bottom-navbar"
      className="fixed bottom-0 left-0 right-0 w-full overflow-x-hidden z-40 safe-area-bottom pt-2 bg-gradient-to-t from-slate-50/90 dark:from-slate-900/90 to-transparent pointer-events-none"
    >
      <div className="max-w-md mx-auto bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/40 dark:border-slate-700/40 shadow-[0_12px_40px_rgba(0,0,0,0.06)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.3)] px-2 py-2 flex items-center justify-between pointer-events-auto">
        {tabs.map((tab) => {
          if (tab.isFab) {
            return (
              <motion.button
